...
def _get_datetime_from_str_date(date_string, date_format = '%m/%d/%Y'):
    date_time = datetime.datetime.strptime(date_string, date_format)
    return date_time

def _convert_timestamp_to_str_date(timestamp, date_format = '%d/%m/%Y', IN_MILLISECONDS = True):
    if IN_MILLISECONDS:
        timestamp_datetime = datetime.datetime.fromtimestamp(timestamp/1000.0)
        return timestamp_datetime.strftime(date_format)
    else:
...
def _convert_string_number_to_readable_number(number, str_format="08,.2f"):
    if float(number) < 0:
        number = float(number) * -1
        if number > 999:
            number = format(number, str_format)
        number = "-" + str(number)
    else:
        if float(number) > 999:
            number = format(number, str_format)
...
def _find_table_entries():
    entry_list = []
    entry_view = wmsDB.db_config.find({ "is_deleted" : False, "config_type" : "TABLE_ENTRIES" })
    for entry_item in entry_view:
