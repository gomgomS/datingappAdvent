...
class view_config_add_webapp_menu:
    ...
    def show_menu_list(self, params):
        ...
        edit_item_rec = database.get_record("db_config_menu_webapp_item_all")
        if value != None:
            edit_item_rec = self.mgdDB.db_config_menu_webapp_item_all.find_one({
                "value" : value
            })
        # end if
        config_all_rec   = self.mgdDB.db_config_all.find_one({"value" : page_id})
        webapp_item_view = self.mgdDB.db_config_menu_webapp_item_all.find({})
        webapp_item_list = list( webapp_item_view )
        response = {
            "webapp_menu_item_list" : webapp_item_list,
    ...
    def html(self, params):
        ...
        webapp_menu_item_list  = show_menu_data["webapp_menu_item_list"] 
        edit_item_rec          = show_menu_data["edit_item_rec" ] 
        config_all_rec         = show_menu_data["config_all_rec"] 
        return render_template(
            "config-menu/menu-list.html",
            menu_list_html        = menu_list,
            webapp_menu_item_list = webapp_menu_item_list,
            edit_item_rec         = edit_item_rec,
            config_all_rec        = config_all_rec,
