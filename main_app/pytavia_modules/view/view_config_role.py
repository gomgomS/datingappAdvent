...
class view_config_role:
    ...
    def show_menu_list(self, params):
        ...
        edit_item_rec = database.get_record("db_config_role")
        if value != None:
            edit_item_rec = self.mgdDB.db_config_role.find_one({
                "value" : value
            })
        # end if
        config_all_rec   = self.mgdDB.db_config_all.find_one({"value" : page_id})
        config_role_view = self.mgdDB.db_config_role.find({})
        config_role_list = list( config_role_view )
        response = {
