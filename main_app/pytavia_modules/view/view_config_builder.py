...
class view_config_builder:
    ...
    def show_menu_list(self, params):
        #prepare params
        page_id         = params["id"] if "id" in params else 'CONFIG_BUILDER'
        ...
        edit_item_rec   = None
        if pkey != None:
            edit_item_rec = self.mgdDB.db_config_vessel_builder.find_one({
                "pkey" : pkey
            })
        #endif
        
        #get data list
        config_data_list = self.mgdDB.db_config_vessel_builder.find({})
