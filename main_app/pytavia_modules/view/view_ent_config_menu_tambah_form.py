...
class view_ent_config_menu_tambah_form :
    ...
    def _find_config_menu(self) :
        config_list = []
        config_view = self.mgdDB.db_ent_config_menu.find({
            "is_deleted" : False }).sort("name", pymongo.ASCENDING)
    ...
    def html(self, params):
        response = helper.response_msg(
            "FIND_ENTERTAINMENT_CONFIG_MENU_FORM_SUCCESS", "FIND ENTERTAINMENT CONFIG MENU FORM SUCCESS", {} , "0000"
        )
        try:
            ...
            # FIND CONFIG 
            config_list = self._find_config_menu()

            html = render_template(
                "entertainment/config_menu_tambah_form.html",
            ...
            response.put( "status"      , "FIND_ENTERTAINMENT_CONFIG_MENU_FORM_FAILED" )
            response.put( "desc"        , "FIND ENTERTAINMENT CONFIG MENU FORM FAILED" )
