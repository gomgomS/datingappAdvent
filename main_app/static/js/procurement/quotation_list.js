...
$(document).ready(function () {
  ...
  var table = $("#tender_list_table").DataTable({
    ...
    columns: [  
      ...
      {data: "sum_of_qty"},
      {data: "submission_end"},
      {data: "negotiation_end"},
      {
        data: {"pkey" : "pkey"},
        mRender: function (data, type, row) {
          
          return "<div class='btn-group'> \
            <a href='/menu/procurement/tenders/final-price?key="+data.pkey+"&st=fp' class='btn btn-xs btn-light mg-l-0' id='opl+" + data.pkey + "'><i class='fa fa-pencil' style='width:30px'></i>  </a> \
            </div>";
        },
      }
    ],
